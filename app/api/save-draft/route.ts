...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (formData) {
      ...
      if (formData.terminals !== undefined) {
      ...
      if (formData.signatureDate) {
        updateData.signature_date = formData.signatureDate
      }
    }

    if (principals !== undefined && Array.isArray(principals)) {
      updateData.principals = principals
      console.log("[v0] Saving principals array with", principals.length, "entries")
    }

    console.log("[v0] Updating fields:", Object.keys(updateData).join(", "))
