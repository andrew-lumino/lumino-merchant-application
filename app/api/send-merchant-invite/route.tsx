...
export async function POST(req: Request) {
  try {
    ...
    if (!inviteId) {
      ...
      return NextResponse.json({ success: false, error: "No invite ID provided" }, { status: 400 })
    }

    if (!agent_email) {
      console.error("❌ No agent email provided")
      return NextResponse.json({ success: false, error: "No agent email provided" }, { status: 400 })
    }
    ...
    const webhookPromises = emails.map((email) => sendZapierWebhook("merchant_application_draft", agent_email, email))
